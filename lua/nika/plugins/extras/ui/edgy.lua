return {
  -- Create and display predefined window layouts.
  {
    ...
    opts = function()
      ...
      if NikaVim.has("neo-tree.nvim") then
        local pos = {
          filesystem = "left",
          buffers = "top",
          git_status = "right",
        ...
        local sources = NikaVim.opts("neo-tree.nvim").sources or {}
        for i, v in ipairs(sources) do
          table.insert(opts.left, i, {
            ...
            pinned = true,
            open = function()
              vim.cmd(("Neotree show position=%s %s dir=%s"):format(pos[v] or "bottom", v, NikaVim.root()))
            end,
          })
        end
      end
