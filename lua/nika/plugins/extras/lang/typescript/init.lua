if nika_docs then
  ...
  ---@type "vtsls" | "tsgo"
  vim.g.nika_ts_lsp = "vtsls" -- currently the default

  -- To use the newer, much faster `tsc` LSP server, either:
  -- * enable the `tsc` extra, or
  -- * set `vim.g.nika_ts_lsp = "tsc"` in your `options.lua`
end

-- change deprecated `tsgo` to `tsc` for now, since with TS7, `tsgo` == `tsc`
if vim.g.nika_ts_lsp == "tsgo" then
  vim.g.nika_ts_lsp = "tsc"
end

local extra = NikaVim.config.register_defaults("ts_lsp", {
