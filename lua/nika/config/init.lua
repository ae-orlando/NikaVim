_G.NikaVim = require("nika.util")

---@class NikaVimConfig: NikaVimOptions
local M = {}

M.version = "16.0.1" -- x-release-please-version
NikaVim.config = M

---@class NikaVimOptions
local defaults = {
  ...
  defaults = {
    autocmds = true, -- nika.config.autocmds
    keymaps = true, -- nika.config.keymaps
    -- nika.config.options can't be configured here since that's loaded before nika setup
    -- if you want to disable loading options, add `package.loaded["nika.config.options"] = true` to the top of your init.lua
  },
  news = {
...
M.json = {
  version = 8,
  loaded = false,
  path = vim.g.nika_json or vim.fn.stdpath("config") .. "/nika.json",
  data = {
...
function M.setup(opts)
  ...
  local group = vim.api.nvim_create_augroup("NikaVim", { clear = true })
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      NikaVim.format.setup()
      NikaVim.news.setup()
      NikaVim.root.setup()

      vim.api.nvim_create_user_command("LazyExtras", function()
        NikaVim.extras.show()
      end, { desc = "Manage NikaVim extras" })
      ...
      if vim.g.nika_check_order == false then
        return
      end
      ...
      local nika_plugins = find("^nika%.plugins$")
      local extras = find("^nika%.plugins%.extras%.", true) or nika_plugins
      local plugins = find("^plugins$") or math.huge
      if nika_plugins ~= 1 or extras > plugins then
        local msg = {
          "The order of your `lazy.nvim` imports is incorrect:",
          "- `nika.plugins` should be first",
          "- followed by any `nika.plugins.extras`",
          "- and finally your own `plugins`",
          "",
          "If you think you know what you're doing, you can disable this check with:",
          "```lua",
          "vim.g.nika_check_order = false",
          "```",
        }
  ...
  NikaVim.track("colorscheme")
  NikaVim.try(function()
...
function M.load(name)
  ...
  local pattern = "NikaVim" .. name:sub(1, 1):upper() .. name:sub(2)
  -- always load nika, then user file
  if M.defaults[name] or name == "options" then
    _load("nika.config." .. name)
    vim.api.nvim_exec_autocmds("User", { pattern = pattern .. "Defaults", modeline = false })
  end
  _load("config." .. name)
  if vim.bo.filetype == "lazy" then
    -- HACK: NikaVim may have overwritten options of the Lazy ui, so reset this here
...
function M.init()
  ...
  M.did_init = true
  local plugin = require("lazy.core.config").spec.plugins.NikaVim
  if plugin then
    vim.opt.rtp:append(plugin.dir)
  end

  package.preload["nika.plugins.lsp.format"] = function()
    NikaVim.deprecate([[require("nika.plugins.lsp.format")]], [[NikaVim.format]])
    return NikaVim.format
  end
  ...
  NikaVim.plugin.setup()
  M.json.load()
end

---@alias NikaVimDefault {name: string, group: string, extra: string, import: string, enabled?: boolean, origin?: "global" | "default" | "extra" }
---
local default_extras ---@type table<string, NikaVimDefault>

---@param name string
---@param extras NikaVimDefault[]
function M.register_defaults(name, extras)
  ...
  local ret ---@type NikaVimDefault?
  local use ---@type string?

  local global = vim.g["nika_" .. name]
  if vim.tbl_contains(valid, global) then
    ...
    if global and global ~= "auto" then
      vim.notify(
        ("Invalid value for `vim.g.nika_%s`: `%s`\nValid options are: %s"):format(
          name,
          global,
          table.concat(valid, ", ")
        ),
    ...
    for _, extra in ipairs(extras) do
      if NikaVim.has_extra(extra.extra) then
  ...
  for _, extra in ipairs(extras) do
    local import = "nika.plugins.extras." .. extra.extra
...
function M.get_defaults()
  ...
  ---@type table<string, NikaVimDefault[]>
  local checks = {
    picker = {
  ...
  if (NikaVim.config.json.data.install_version or 7) < 8 then
