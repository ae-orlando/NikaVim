...
---@class nika.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "NikaVim", desc = "NikaVim extras", module = "nika.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("nika.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #NikaVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = NikaVim.find_root(source.module)
    if root then
      NikaVim.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  NikaVim.plugin.handle_defaults = false
  ...
  local defaults = NikaVim.config.get_defaults()
  local def = defaults[modname]
  if def and def.enabled == false and vim.startswith(modname, "nika.plugins.extras.lang.") then
    recommended = false
  end
  ...
  return {
    ...
    managed = vim.tbl_contains(NikaVim.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X.new()
  ...
  self.float = Float.new({ title = "NikaVim Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      NikaVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, NikaVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(NikaVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(NikaVim.config.json.data.extras)
      NikaVim.json.save()
      NikaVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart NikaVim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("NikaVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled NikaVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(NikaVim.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic({
  ...
  if extra.source.name ~= "NikaVim" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^nika.plugins.extras.", "")
