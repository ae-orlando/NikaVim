...
---@return boolean ok, nika.util.treesitter.Health health
function M.check()
  ...
  ---@class nika.util.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      NikaVim.error(lines, { title = "NikaVim Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      NikaVim.info("Installing `tree-sitter-cli` with `mason.nvim`...")
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            NikaVim.info("Installed `tree-sitter-cli` with `mason.nvim`.")
            cb(true)
          else
