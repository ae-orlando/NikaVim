...
function M.formatter(opts)
  ...
  local ret = {
    ...
    priority = 1,
    format = function(buf)
      M.format(NikaVim.merge({}, filter, { bufnr = buf }))
    end,
    sources = function(buf)
      local clients = vim.lsp.get_clients(NikaVim.merge({}, filter, { bufnr = buf }))
  ...
  return NikaVim.merge(ret, opts) --[[@as LazyFormatter]]
end
...
function M.format(opts)
  opts = vim.tbl_deep_extend("force", {}, opts or {}, NikaVim.opts("nvim-lspconfig").format or {})
...
function M.execute(opts)
  ...
  ---@cast filter vim.lsp.get_clients.Filter
  local client = vim.lsp.get_clients(NikaVim.merge({}, filter, { bufnr = buf }))[1]
