...
function M.formatexpr()
  if NikaVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  NikaVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    NikaVim.warn("No formatter available", { title = "NikaVim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "nika.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    NikaVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in NikaVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `nika.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return NikaVim.format.enabled()
    end,
    set = function(state)
      NikaVim.format.enable(state, buf)
    end,
  })
end
